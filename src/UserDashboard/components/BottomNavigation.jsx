...
const BottomNavigation = () => {
  ...
  const isBooking = location.pathname === "/user/booking";

  const isWideSuitablePage = isDashboard || isSchedule || isBooking;

  if (isWideSuitablePage) {
    return (
      <div className="fixed bottom-0 left-0 w-full bg-white border-t border-slate-200/80 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-50 transition-all duration-300">
        <div className="max-w-4xl mx-auto flex justify-around items-center py-2.5 md:py-3.5 px-6">
          {/* Home */}
          <button
            onClick={() => navigate("/user/dashboard")}
            className={`flex flex-col items-center justify-center rounded-2xl px-5 py-1.5 transition-all duration-250 cursor-pointer hover:scale-105 active:scale-95 ${
              isHome
  ...
  // Otherwise, render the current capsule bottom navigation
  return (
